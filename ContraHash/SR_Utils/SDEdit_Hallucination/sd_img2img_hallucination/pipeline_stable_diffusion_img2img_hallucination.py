...
# [NeurIPS24] Understanding Hallucinations in Diffusion Models through Mode Interpolation 
# Class for tracking the variance and mean of the trajectories. 
class HalCalculator:
    """
    ...
    def __init__(self, timestep_range: int = 200):
...
class StableDiffusionImg2ImgPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # dictionary for saving the intermediate results [for NeurIPS24 paper]
        # intermediates = {
        #     'pred_x0': [],  # would be initial latents 
        # }
        Hal_x0 = None 
        # if isinstance(self.scheduler, DDIMScheduler):
        hallucinator = HalCalculatorTensor(shape=latents.shape, timestep_range=len(range(num_inference_steps)))
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # [NeurIPS24] Understanding Hallucinations in Diffusion Models through Mode Interpolation
                # Apply the hallucination metrics for further reject the sampled image 
                # [1] compute the intermediates pred_x0 
                # alpha_prod = self.scheduler.alphas_cumprod[t]
                # sqrt_alpha = torch.sqrt(alpha_prod)
                # sqrt_one_minus_alpha = torch.sqrt(1 - alpha_prod)
                # # pred_x0 
                # pred_x0 = (latents - sqrt_one_minus_alpha * noise_pred) / sqrt_alpha
                

                # [2] compute the previous noisy sample x_t -> x_t-1
                # if DDIMScheduler 
                if isinstance(self.scheduler, DDIMScheduler):
                    latents, pred_x0 = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)
                    # intermediates['pred_x0'].append(pred_x0)
                    Hal_x0 = hallucinator.add(pred_x0.cpu().detach())
                # Added PNDMScheduler support 
                else: 
                    _sampled = self.scheduler.step_plms_with_pred_x0(noise_pred, t, latents, **extra_step_kwargs)
                    latents  = _sampled['prev_sample']
                    Hal_x0   = hallucinator.add(_sampled['pred_original_sample'].cpu().detach())
                    # latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]
                # standard diffuser pipeline 
        ...
        if Hal_x0 is not None:
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), \
                Hal_x0.mean().item(), hallucinator.trajectory
        else: 
            return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
